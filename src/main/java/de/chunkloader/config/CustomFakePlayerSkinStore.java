...
public final class CustomFakePlayerSkinStore {
    ...
    public synchronized void load() {
        ...
        try {
            ...
            LOGGER.warn("Unable to create custom skin directory {}: {}", skinsDir, e.getMessage());
            return;
        }
        ...
        try {
            ...
            for (Map.Entry<String, JsonElement> entry : entries.entrySet()) {
                ...
                if (!isValidSkinPng(pngBytes)) {
                    LOGGER.warn("Skipping invalid stored skin PNG for '{}'", name);
                    continue;
                }
                skinsByName.put(name, new StoredSkin(name, layerMask, model != null ? model : "", pngBytes));
            }
        } catch (Exception e) {
            LOGGER.warn("Failed to load custom skins meta {}: {}", metaPath, e.getMessage());
        }
    }
    ...
    public synchronized boolean remove(String playerName) {
        ...
        try {
            ...
            LOGGER.warn("Failed to delete custom skin file for '{}': {}", name, e.getMessage());
        }
        return true;
    }
    ...
    public synchronized StoredSkin rename(String oldPlayerName, String newPlayerName) {
        ...
        try {
            ...
            LOGGER.warn("Failed to rename custom skin '{}' -> '{}': {}", oldName, newName, e.getMessage());
        ...
        try {
            ...
            LOGGER.warn("Failed to save skins meta after rename: {}", e.getMessage());
        }
        return renamed;
    }
