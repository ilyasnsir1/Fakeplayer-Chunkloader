...
public class ChunkloaderCommand {
    ...
    private static int renameChunkloader(CommandContext<CommandSourceStack> context) {
        String name = StringArgumentType.getString(context, "name");
        String newNameRaw = StringArgumentType.getString(context, "newName");
        
        if (newNameRaw == null || newNameRaw.trim().isEmpty()) {
            context.getSource().sendFailure(Component.literal("New name cannot be empty").withStyle(ChatFormatting.RED));
            return 0;
        }
        
        final String newName = newNameRaw.trim();
