...
public final class ChunkloaderClientNetworking {
    ...
    public static void sendAction(ChunkloaderActionPayload.Action action, int chunkX, int chunkZ, String dimension, int value) {
        PacketDistributor.sendToServer(new ChunkloaderActionPayload(chunkX, chunkZ, dimension, action, value));
    }

    public static void requestSimulationStatus() {
        PacketDistributor.sendToServer(new SimulationStatusRequestPayload(false));
    }

    public static void requestChunkplayerStatus() {
        PacketDistributor.sendToServer(new ChunkplayerStatusRequestPayload(false));
    }

    public static void requestDisabledChunkloadersList() {
        PacketDistributor.sendToServer(new DisabledChunkloadersListRequestPayload());
    }

    public static void sendDeleteDisabledChunkloader(int chunkX, int chunkZ, String dimension) {
        PacketDistributor.sendToServer(new DeleteDisabledChunkloaderPayload(chunkX, chunkZ, dimension));
    }

    public static void sendRestoreDisabledChunkloader(int chunkX, int chunkZ, String dimension) {
        PacketDistributor.sendToServer(new RestoreDisabledChunkloaderPayload(chunkX, chunkZ, dimension));
    }
    ...
    public static void sendUpdateDisabledChunkloaderCoords(int oldChunkX, int oldChunkZ, String oldDimension, int newChunkX, int newChunkZ, int newBlockX, int newBlockY, int newBlockZ) {
        PacketDistributor.sendToServer(new UpdateDisabledChunkloaderCoordsPayload(oldChunkX, oldChunkZ, oldDimension, newChunkX, newChunkZ, newBlockX, newBlockY, newBlockZ));
    }

    public static void sendRenameChunkloader(int chunkX, int chunkZ, String dimension, String newName) {
        PacketDistributor.sendToServer(new RenameChunkloaderPayload(chunkX, chunkZ, dimension, newName));
    }

    public static void sendCloseChunkMapToServer() {
        PacketDistributor.sendToServer(new CloseChunkMapRequestPayload());
    }
    ...
    public static void sendClearCustomSkin(String playerName) {
        PacketDistributor.sendToServer(new de.chunkloader.network.payload.ClearCustomSkinPayload(playerName));
    }
}
