...
public class ChunkMapScreen extends Screen {
    ...
    private List<Component> pendingDotTooltip = null;
            ...
            if (!isSkinScreenPage) {
                ...
                if (this.grid != null) {
                    ...
                    if (colorEditorOverlay != null) {
                        ...
                        updatePendingDotTooltip(widgetMouseX, widgetMouseY);
                        int cellMouseX = pendingDotTooltip != null ? -999 : widgetMouseX;
                        int cellMouseY = pendingDotTooltip != null ? -999 : widgetMouseY;
                        grid.render(context, cellMouseX, cellMouseY);
                        drawChunkloaderPoints(context);
                    }
                ...
                if (colorEditorOverlay == null) {
                    if (pendingDotTooltip != null) {
                        drawSimpleTooltip(context, pendingDotTooltip, mouseX, mouseY);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (colorEditorOverlay != null) {
            return colorEditorOverlay.mouseClicked(mouseX, mouseY, button);
        }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        if (colorEditorOverlay != null) {
            return colorEditorOverlay.mouseDragged(mouseX, mouseY, button);
        }
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if (colorEditorOverlay != null) {
            return colorEditorOverlay.mouseReleased(mouseX, mouseY, button);
        }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (colorEditorOverlay != null) {
            return colorEditorOverlay.keyPressed(keyCode, scanCode, modifiers);
        }
    ...
    private ResourceLocation getPlayerSkinTexture(String playerName) {
        ...
        ResourceLocation entryTexture = resolveSkinFromPlayerInfo(client, playerName);
    ...
    private ResourceLocation resolveSkinFromPlayerInfo(Minecraft client, String playerName) {
        ...
        for (var playerEntry : handler.getOnlinePlayers()) {
            if (playerEntry.getProfile().getName() != null && playerEntry.getProfile().getName().equals(playerName)) {
                entry = playerEntry;
                break;
            }
        }
    ...
    private void drawSimpleTooltip(GuiGraphics graphics, List<Component> lines, int mouseX, int mouseY) {
        ...
        if (x + boxW > this.width) {
            x = Math.max(2, this.width - boxW - 2);
        }
        if (y + boxH > this.height) {
            y = Math.max(2, this.height - boxH - 2);
        }
        ...
        for (Component c : lines) {
            if (c != null) {
                graphics.drawString(this.font, c, x + padding, ty, 0xFFFFFFFF, false);
            }
            ty += lineHeight;
        }
    }
    ...
    private static void blitNonAtlas(
        ...
        try {
            g.blit(texture, x, y, width, height, u, v, uWidth, vHeight, textureWidth, textureHeight);
    ...
    private void updatePendingDotTooltip(int mouseX, int mouseY) {
        pendingDotTooltip = null;
        ...
        if (ownColumn >= 0 && ownColumn < data.mapWidth() && ownRow >= 0 && ownRow < data.mapHeight()) {
            ...
            if (mouseX >= ownPointX - ownPointHalfSize - 2 && mouseX < ownPointX + ownPointHalfSize + 2 &&
                ...
                pendingDotTooltip = List.of(Component.literal(ownTooltipText));
                return;
            }
        }

        if (data.otherChunkloaders() == null || data.otherChunkloaders().isEmpty() || clientConfig.isHideOtherDots()) {
            return;
        }
        ...
        for (de.chunkloader.network.ChunkloaderPosition pos : data.otherChunkloaders()) {
            ...
            if (mouseX >= pointX - pointHalfSize - 2 && mouseX < pointX + pointHalfSize + 2 &&
                ...
                pendingDotTooltip = List.of(Component.literal(tooltipText));
                return;
            }
        }
    }

    private void drawChunkloaderPoints(GuiGraphics context) {
        int cellSize = CELL_SIZE;
        int gridInnerLeft = gridLeft + 1;
        int gridInnerTop = gridTop + 1;
