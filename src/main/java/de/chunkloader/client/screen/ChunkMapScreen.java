...
public class ChunkMapScreen extends Screen {
    ...
    private static final Identifier GRID_OVERLAY = Identifier.fromNamespaceAndPath("chunkloader", "textures/gui/grid_overlay.png");
    private static final Identifier HOVER_OVERLAY = Identifier.fromNamespaceAndPath("chunkloader", "textures/gui/cell_overlay.png");
    ...
    public void removed() {
        ...
        if (!retainChunkMapSession) {
            ChunkloaderClientNetworking.sendCloseChunkMapToServer();
            if (this.grid != null) {
                this.grid.close();
                this.grid = null;
            }
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
            ...
            if (!isSkinScreenPage) {
                ...
                if (this.grid != null) {
                    ...
                    if (colorEditorOverlay != null) {
                        ...
                        grid.extractRenderState(context, widgetMouseX, widgetMouseY);
                    ...
                    int compassX = gridLeft + gridWidth / 2 - font.width(upLabel) / 2;
                    int compassY = gridTop - font.lineHeight - 8;
                    int compassColor = toGuiFillColor(clientConfig != null ? clientConfig.getCompassDirectionColor() : 0xFFFFFFAA);
                    context.text(font, Component.literal(upLabel), compassX, compassY, compassColor, true);
                }
                ...
                if (actionSearchField != null) {
                    ...
                    actionSearchField.extractRenderState(context, widgetMouseX, widgetMouseY, delta);
                }
                ...
                for (Button button : actionButtons) {
                    if (button != resetButton) {
                        int bTop = button.getY();
                        int bBottom = bTop + button.getHeight();
                        if (bBottom >= scissorTop && bTop <= actionViewportBottom) {
                            button.extractRenderState(context, widgetMouseX, widgetMouseY, delta);
                        }
                    }
                }
                context.disableScissor();
                ...
                if (resetButton != null) {
                    resetButton.extractRenderState(context, widgetMouseX, widgetMouseY, delta);
                }

                if (closeButton != null) {
                    closeButton.extractRenderState(context, widgetMouseX, widgetMouseY, delta);
                }

                if (colorEditorOverlay == null) {
                    ChunkMapGrid.Cell hovered = grid != null ? grid.getHoveredCell(mouseX, mouseY) : null;
                    if (hovered != null) {
                        context.setTooltipForNextFrame(font, hovered.buildTooltip(), java.util.Optional.empty(), mouseX, mouseY);
        ...
        if (colorEditorOverlay != null) {
            colorEditorOverlay.extractRenderState(context, mouseX, mouseY);
        }
    }
    ...
    private void drawActionScrollbar(GuiGraphicsExtractor context) {
        int maxScroll = getMaxActionScroll();
        if (maxScroll <= 0) {
            return;
        }
    ...
    private void drawActionHeaders(GuiGraphicsExtractor context) {
        ...
        for (ActionHeaderLayout headerLayout : actionHeaderLayouts) {
            ...
            context.text(this.font, headerLayout.text, x, textY, textColor, false);
        }
    }
    ...
    private void drawMapFrame(GuiGraphicsExtractor context) {
        int framePadding = 6;
    ...
    private void drawActionSearchFieldChrome(GuiGraphicsExtractor context) {
        if (actionSearchField == null) {
            return;
        }
    ...
    private void applyActionSearchFieldStyle() {
        ...
        actionSearchField.setHint(Component.literal("Search...").withStyle(style -> style.withColor(TextColor.fromRgb(placeholderRgb))));
    }
    ...
    private int lastTargetHitMouseX = Integer.MIN_VALUE;
    private int lastTargetHitMouseY = Integer.MIN_VALUE;
    private int lastTargetHitScroll = Integer.MIN_VALUE;
    private int lastTargetHitRotation = -1;
    private PanelColorTargetHit cachedTargetHit = null;

    public PanelColorTargetHit getPanelColorTargetHit(int mouseX, int mouseY) {
        if (lastTargetHitMouseX == mouseX && lastTargetHitMouseY == mouseY && lastTargetHitScroll == actionScrollOffset && lastTargetHitRotation == mapRotation) {
            return cachedTargetHit;
        }

        PanelColorTargetHit hit = computePanelColorTargetHit(mouseX, mouseY);
        lastTargetHitMouseX = mouseX;
        lastTargetHitMouseY = mouseY;
        lastTargetHitScroll = actionScrollOffset;
        lastTargetHitRotation = mapRotation;
        cachedTargetHit = hit;
        return hit;
    }
    ...
    private void collectActionHeaderHits(List<PanelColorTargetHit> hits) {
        ...
        int buttonWidth = panelWidth - 16;
        for (ActionHeaderLayout headerLayout : actionHeaderLayouts) {
            int rowY = actionViewportTop + headerLayout.yOffset - actionScrollOffset;
            int textY = rowY + (headerLayout.height - font.lineHeight) / 2 + 1;
            int textWidth = font.width(headerLayout.text);
            int textX = buttonX + Math.max(0, (buttonWidth - textWidth) / 2);
            int textTop = textY - 1;
            int textBottom = textY + font.lineHeight + 1;
    ...
    private void collectInfoDimHits(List<PanelColorTargetHit> hits) {
        ...
        int dimTextWidth = font.width(dimText);
        addTextHit(
            hits,
    ...
    private void collectInfoNameHits(List<PanelColorTargetHit> hits) {
        ...
        int nameWidth = font.width(layout.ownerName);
    ...
    private void collectSearchbarHits(List<PanelColorTargetHit> hits, PanelColorTarget target) {
        ...
        switch (target) {
            ...
            case SEARCHBAR_TEXT -> {
                ...
                int textTop = top + ACTION_SEARCH_INSET_Y + Math.max(0, (ACTION_SEARCH_TEXT_HEIGHT - font.lineHeight) / 2);
                ...
                if (!query.isBlank()) {
                    hits.add(hit(
                        ...
                        textLeft,
                        textTop - 1,
                        textLeft + font.width(query) + 3,
                        textTop + font.lineHeight + 1
                    ));
                } else {
            ...
            case SEARCHBAR_PLACEHOLDER -> {
                ...
                hits.add(hit(
                    ...
                    textLeft,
                    textTop - 1,
                    textLeft + font.width(placeholder) + 3,
                    textTop + font.lineHeight + 1
                ));
            }
            default -> {
    ...
    private void collectCompassHit(List<PanelColorTargetHit> hits) {
        ...
        hits.add(hit(
            ...
            compassX - 3,
            compassY - 2,
            compassX + font.width(upLabel) + 3,
            compassY + font.lineHeight + 2
        ));
    }

    private void addTextHit(List<PanelColorTargetHit> hits, PanelColorTarget target, String text, int x, int y) {
        hits.add(hit(target, x - 2, y - 2, x + font.width(text) + 1, y + font.lineHeight));
    }
    ...
    private InfoLayout getInfoLayout() {
        ...
        int initialInfoY = nameY > 0 ? nameY + font.lineHeight + 8 : headY + headSize + 8;
        ...
        int maxLabelWidth = Math.max(font.width("Chunk:"), font.width("Block:"));
    ...
    private PanelColorTargetHit computePanelColorTargetHit(int mouseX, int mouseY) {
    ...
    private PanelColorTargetHit getInfoTextColorHit(int mouseX, int mouseY) {
        ...
        if (ownerName != null && !ownerName.isEmpty()) {
            int nameWidth = font.width(ownerName);
            int nameX = leftPanelX + (leftPanelWidth - nameWidth) / 2;
            nameY = headY + headSize + 4;
        ...
        int infoY = nameY > 0 ? nameY + font.lineHeight + 8 : headY + headSize + 8;
        infoY += 2;
        ...
        int maxLabelWidth = Math.max(font.width(chunkLabel), font.width(blockLabel));
        int coordStartX = leftPanelX + padding + maxLabelWidth + 4;
    ...
    private PanelColorTargetHit hitText(PanelColorTarget target, int mouseX, int mouseY, String text, int x, int y) {
        return hitIfContains(target, mouseX, mouseY, x - 2, y - 2, x + font.width(text) + 1, y + font.lineHeight);
    }
    ...
    private void drawTopBox(GuiGraphicsExtractor context) {
        int panelColor = toGuiFillColor(clientConfig != null ? clientConfig.getPanelColor() : 0xFF2C2C2C);
    ...
    private void drawLeftPanel(GuiGraphicsExtractor context) {
        int panelColor = toGuiFillColor(clientConfig != null ? clientConfig.getPanelColor() : 0xFF2C2C2C);
        int borderColor = toGuiFillColor(clientConfig != null ? clientConfig.getBorderColor() : 0xFF4A4A4A);
        ...
        if (ownerName != null && !ownerName.isEmpty()) {
            ...
            Component nameText = Component.literal(ownerName);
                ...
                int nameX = leftPanelX + (leftPanelWidth - nameWidth) / 2;
                nameY = headY + headSize + 4;
                context.text(this.font, nameText, nameX, nameY, leftPanelNameColor, false);
            } else {
            nameY = 0;
        }
        ...
        context.text(this.font, Component.literal("Status:"), leftPanelX + padding, infoY, leftPanelTextColor, false);
        String statusText = data.enabled() ? "active" : "inactive";
        ...
        context.text(this.font, Component.literal("Dim:"),
        ...
        context.text(this.font, Component.literal(dimText),
        ...
        int chunkBlockY = infoY - 3;
        context.text(this.font, chunkLabel, leftPanelX + padding, chunkBlockY, leftPanelTextColor, false);

        String chunkXStr = "X:" + data.centerChunkX();
        String chunkZStr = "Z:" + data.centerChunkZ();

        context.text(this.font, Component.literal(chunkXStr), coordStartX, chunkBlockY, leftPanelValueColor, false);
        chunkBlockY += 12;
        context.text(this.font, Component.literal(chunkZStr), coordStartX, chunkBlockY, leftPanelValueColor, false);
        chunkBlockY += 12;

        BlockPos blockPos = new BlockPos(data.centerChunkX() << 4, data.blockY(), data.centerChunkZ() << 4);

        context.text(this.font, blockLabel, leftPanelX + padding, chunkBlockY, leftPanelTextColor, false);

        String xStr = "X:" + blockPos.getX();
        String yStr = "Y:" + data.blockY();
        String zStr = "Z:" + blockPos.getZ();

        context.text(this.font, Component.literal(xStr), coordStartX, chunkBlockY, leftPanelValueColor, false);
        chunkBlockY += 12;
        context.text(this.font, Component.literal(yStr), coordStartX, chunkBlockY, leftPanelValueColor, false);
        chunkBlockY += 12;
        context.text(this.font, Component.literal(zStr), coordStartX, chunkBlockY, leftPanelValueColor, false);
        ...
        String radiusLabel = data.allowMobSpawning() ? "SD:" : "Radius:";
        int sdY = sdDividerY + 8;
        context.text(this.font, Component.literal(radiusLabel),
                leftPanelX + padding, sdY, leftPanelTextColor, false);
            context.text(this.font, Component.literal(radiusValue),
                coordStartX, sdY, leftPanelValueColor, false);
            int radiusValueWidth = this.font.width(radiusValue);
            context.text(this.font, Component.literal(radiusSeparator),
    ...
    private void drawSidePanel(GuiGraphicsExtractor context) {
        ...
        context.text(renderer, title.withStyle(net.minecraft.ChatFormatting.BOLD), panelX + padding, y, 0xFFFFFF, false);
        y += 16;

        drawSeparator(context, panelX + padding, y, panelWidth - padding * 2);
        y += 8;

        context.text(renderer, Component.literal("Status:").withStyle(net.minecraft.ChatFormatting.GRAY),
        ...
        int statusColor = data.enabled()
            ...
            : 0xFF5555;
        context.text(renderer, status, panelX + padding + 50, y, statusColor, false);
        y += 12;

        context.text(renderer, Component.literal("Mode:").withStyle(net.minecraft.ChatFormatting.GRAY),
            panelX + padding, y, 0xFFFFFF, false);
        ...
        context.text(renderer, mode, panelX + padding + 50, y, modeColor, false);
        y += 12;

        drawSeparator(context, panelX + padding, y, panelWidth - padding * 2);
        y += 8;

        context.text(renderer, Component.literal("Dimension:").withStyle(net.minecraft.ChatFormatting.GRAY),
            panelX + padding, y, 0xFFFFFF, false);
        context.text(renderer, Component.literal(data.dimensionKey()), panelX + padding + 50, y, 0xFFFFFF, false);
        y += 12;

        BlockPos blockPos = new BlockPos(data.centerChunkX() << 4, data.blockY(), data.centerChunkZ() << 4);
        context.text(renderer, Component.literal("Position:").withStyle(net.minecraft.ChatFormatting.GRAY),
            panelX + padding, y, 0xFFFFFF, false);
        context.text(renderer, Component.literal(blockPos.getX() + " / " + data.blockY() + " / " + blockPos.getZ()),
            panelX + padding + 50, y, 0xFFFFFF, false);
        y += 12;

        String radiusLabelSide = data.allowMobSpawning() ? "SD:" : "Radius:";
        context.text(renderer, Component.literal(radiusLabelSide).withStyle(net.minecraft.ChatFormatting.GRAY),
            panelX + padding, y, 0xFFFFFF, false);
        MutableComponent radiusText = Component.literal(data.chunkRadius() + " / 3");
        context.text(renderer, radiusText, panelX + padding + 50, y, 0xFFFFFF, false);
            y += 12;
        ...
        if (legendStartY + legendHeight > buttonAreaStart - 10) {
            ...
            context.text(renderer, Component.literal("Legend").withStyle(net.minecraft.ChatFormatting.BOLD),
            panelX + padding, y, 0xFFFFFF, false);
        y += 12;
    ...
    private Identifier resolveSkinFromPlayerListEntry(Minecraft client, String playerName) {
    ...
    private void drawPlayerHead(GuiGraphicsExtractor context, int x, int y, int size, String playerName) {
    ...
    private void drawLegendItem(GuiGraphicsExtractor context, Font renderer, int x, int y, int color, Component text) {
        int squareSize = 6;
        ...
        context.text(renderer, text, x + 10, y, 0xFFFFFF, false);
    }

    private void drawChunkloaderPoints(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        ...
        if (ownColumn >= 0 && ownColumn < data.mapWidth() && ownRow >= 0 && ownRow < data.mapHeight()) {
            ...
            if (mouseX >= ownPointX - ownPointHalfSize - 2 && mouseX < ownPointX + ownPointHalfSize + 2 &&
                ...
                context.setTooltipForNextFrame(Component.literal(ownTooltipText), mouseX, mouseY);
            }
        }
        ...
        for (de.chunkloader.network.ChunkloaderPosition pos : data.otherChunkloaders()) {
            ...
            if (mouseX >= pointX - pointHalfSize - 2 && mouseX < pointX + pointHalfSize + 2 &&
                ...
                context.setTooltipForNextFrame(Component.literal(tooltipText), mouseX, mouseY);
            }
        }
    }
        ...
        void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY) {
            ...
            for (Cell cell : cells) {
                if (cell.left + cell.size >= scissorLeft && cell.left <= scissorRight &&
                    ...
                    cell.extractRenderState(context, mouseX, mouseY);
                }
            }
        }
            ...
            btn -> {
                openChildScreen(new ChunkloaderConfirmationScreen(
                    ...
                    () -> {
                        ...
                        ChunkloaderClientNetworking.sendAction(
    ...
    private void buildActionButtons() {
        ...
        Button mobButton = Button.builder(
            ...
            btn -> ChunkloaderClientNetworking.sendAction(
