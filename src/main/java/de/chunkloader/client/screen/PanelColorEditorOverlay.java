...
final class PanelColorEditorOverlay {
    ...
    private void renderVanillaButton(
        ...
        context.blitSprite(RenderPipelines.GUI_TEXTURED, VANILLA_BUTTON_TEXTURES.get(active, hovered), x1, y1, width, height);
    ...
    private void ensureTextures() {
        ...
        for (int row = 0; row < PALETTE_ROWS; row++) {
            for (int col = 0; col < PALETTE_COLUMNS; col++) {
                horizontal.setPixel(col, row, 0xFF000000 | getPaletteRgb(col, row));
            }
        }
        horizontalPaletteTexture = new DynamicTexture(() -> "chunkloader_editor_palette_h", horizontal);
        ...
        for (int row = 0; row < PALETTE_COLUMNS; row++) {
            for (int col = 0; col < PALETTE_ROWS; col++) {
                vertical.setPixel(col, row, 0xFF000000 | getPaletteRgb(row, col));
            }
        }
        verticalPaletteTexture = new DynamicTexture(() -> "chunkloader_editor_palette_v", vertical);
        ...
        for (int y = 0; y < 12; y++) {
            for (int x = 0; x < 12; x++) {
                ...
                checker.setPixel(x, y, light ? 0xFFFFFFFF : 0xFFB0B0B0);
            }
        }
        checkerboardTexture = new DynamicTexture(() -> "chunkloader_editor_checker", checker);
    ...
    private void ensureOpacityGradientTexture(int rgb, int width, int height, boolean vertical) {
        ...
        opacityGradientTexture = new DynamicTexture(() -> "chunkloader_editor_opacity", image);
    ...
    private static void fillOpacityGradient(NativeImage image, int rgb, int width, int height, boolean vertical) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                image.setPixel(x, y, (alphaValue << 24) | (rgb & 0x00FFFFFF));
            }
        }
    }
