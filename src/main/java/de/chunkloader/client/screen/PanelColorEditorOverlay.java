...
final class PanelColorEditorOverlay {
    ...
    boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (hexField != null && hexField.isMouseOver(mouseX, mouseY)) {
            focusedControl = 0;
            updateFocusState();
            hexField.mouseClicked(mouseX, mouseY, 0);
            return true;
    ...
    private void renderVanillaButton(
        ...
        context.drawGuiTexture(texture, x1, y1, Math.max(1, x2 - x1), Math.max(1, y2 - y1));
    ...
    private PanelColorTarget getSkinPreviewTargetAt(int mouseX, int mouseY) {
        ...
        int cw = font().getWidth("Drag: rotate  |  Double-click: reset");
    ...
    private void ensureTextures() {
        ...
        for (int row = 0; row < PALETTE_ROWS; row++) {
            for (int col = 0; col < PALETTE_COLUMNS; col++) {
                horizontal.setColor(col, row, NativeImageColorUtil.argbToNative(0xFF000000 | getPaletteRgb(col, row)));
            }
        }
        horizontalPaletteTexture = new NativeImageBackedTexture(horizontal);
        ...
        for (int row = 0; row < PALETTE_COLUMNS; row++) {
            for (int col = 0; col < PALETTE_ROWS; col++) {
                vertical.setColor(col, row, NativeImageColorUtil.argbToNative(0xFF000000 | getPaletteRgb(row, col)));
            }
        }
        verticalPaletteTexture = new NativeImageBackedTexture(vertical);
        ...
        for (int y = 0; y < 12; y++) {
            for (int x = 0; x < 12; x++) {
                ...
                checker.setColor(x, y, NativeImageColorUtil.argbToNative(light ? 0xFFFFFFFF : 0xFFB0B0B0));
            }
        }
        checkerboardTexture = new NativeImageBackedTexture(checker);
    ...
    private void ensureOpacityGradientTexture(int rgb, int width, int height, boolean vertical) {
        ...
        opacityGradientTexture = new NativeImageBackedTexture(image);
    ...
    private static void fillOpacityGradient(NativeImage image, int rgb, int width, int height, boolean vertical) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                image.setColor(x, y, NativeImageColorUtil.argbToNative((alphaValue << 24) | (rgb & 0x00FFFFFF)));
            }
        }
    }
