...
final class PanelColorEditorOverlay {
    ...
    void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        ensureTextures();
        EditorLayout layout = getLayout();
        ...
        if (layout.vertical()) {
            if (currentTarget == null) {
                ...
                context.text(font(), targetLabel, labelX, labelY, 0xFFAAAAAA, false);
            } else {
                context.text(font(), Component.literal("Color: (A:" + alpha(currentColor) + ")").withStyle(ChatFormatting.GRAY), layout.cardLeft(), layout.cardTop() + 2, 0xFFAAAAAA, false);
                String[] words = currentTarget.getDisplayName().split(" ");
                int curY = layout.cardTop() + 2 + font().lineHeight + 1;
                for (String word : words) {
                    context.text(font(), Component.literal(word).withStyle(ChatFormatting.WHITE), layout.cardLeft(), curY, 0xFFFFFFFF, false);
            ...
            context.text(font(), targetLabel, labelX, labelY, 0xFFFFFFFF, false);
        }
        ...
        context.text(font(), Component.literal("<").withStyle(ChatFormatting.BOLD), leftArrowX + (arrowW - font().width("<")) / 2, arrowY + (arrowH - font().lineHeight) / 2, leftColor, false);

        int rightColor = rightHovered ? 0xFFFFDD55 : 0xBBFFFFFF;
        context.text(font(), Component.literal(">").withStyle(ChatFormatting.BOLD), rightArrowX + (arrowW - font().width(">")) / 2, arrowY + (arrowH - font().lineHeight) / 2, rightColor, false);
        ...
        if (hexField != null) {
            ...
            hexField.extractRenderState(context, mouseX, mouseY, 0.0F);
        }
    }
    ...
    boolean mouseClicked(MouseButtonEvent click) {
        ...
        if (getHelpMarkBounds().contains(mouseX, mouseY)) {
            Minecraft.getInstance().gui.setScreen(new PanelColorHelpScreen(owner));
            return true;
        }
    ...
    boolean mouseDragged(MouseButtonEvent click) {
    ...
    boolean keyPressed(KeyEvent keyInput) {
        ...
        if (key == InputConstants.KEY_TAB) {
            boolean shift = keyInput.hasShiftDown();
            if (focusedControl == -1) {
        ...
        if ((key == InputConstants.KEY_RETURN || key == InputConstants.KEY_SPACE) && focusedControl > 0) {
            if (focusedControl == 1 && hasUnsavedChanges()) {
                config.savePanelColors(draftColors);
                unsavedChangesCache = false;
                owner.closePanelColorEditor();
        ...
        if (hexField != null && hexField.isFocused()) {
            if (key == InputConstants.KEY_BACKSPACE && hexField.getCursorPosition() <= 1) {
                return true;
            }
    ...
    boolean charTyped(net.minecraft.client.input.CharacterEvent characterEvent) {
        if (hexField != null && hexField.isFocused()) {
            return hexField.charTyped(characterEvent);
        }
        return false;
    }

    private void renderPalette(GuiGraphicsExtractor context, int mouseX, int mouseY, EditorLayout layout) {
    ...
    private void renderOpacitySlider(GuiGraphicsExtractor context, EditorLayout layout, int color) {
    ...
    private void renderVanillaButton(
        ...
        Button btn = Button.builder(Component.literal(text), b -> {})
            .bounds(x1, y1, Math.max(1, x2 - x1), Math.max(1, y2 - y1))
            .build();
        btn.active = active;
        btn.extractRenderState(context, mouseX, mouseY, 0);
    }
    ...
    private void drawSkinScreenPreview(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        int panelWidth = getPreviewPanelWidth();
        int panelHeight = Math.min(370, Math.max(1, owner.height - 24));
        int panelX = getPreviewPanelX(panelWidth);
        int panelY = (owner.height - panelHeight) / 2;
        ...
        int titleY = panelY + 12;
        context.text(font(), title, titleX, titleY, skinTitleColor, false);

        context.text(font(), Component.literal("Player: Fakeplayer4"), panelX + 18, panelY + 35, skinPlayerNameColor, false);
        context.text(font(), Component.literal("Skin PNG"), panelX + 18, panelY + 52, skinTextColor, false);
        ...
        context.text(font(), Component.literal("Path to a skin PNG"), rowX + 4, fileFieldY + 7, skinPathPlaceholderColor, false);
        int pathLabelW = font().width("Path to a skin PNG");
        context.text(font(), Component.literal("/custom.png"), rowX + 6 + pathLabelW, fileFieldY + 7, skinPathTextColor, false);
        ...
        context.text(font(), Component.literal("3D Preview"), previewLeft, previewTop - 13, skinTextColor, false);
        ...
        context.text(font(), hint, previewLeft + (previewWidth - font().width(hint)) / 2, previewTop + previewHeight / 2 - 4, skinTextColor, false);
        ...
        context.text(font(), controlsHint, previewLeft + (previewWidth - font().width(controlsHint)) / 2, previewBottom + 8, skinTextColor, false);

        int statusY = panelY + panelHeight - 58;
        String statusText = getSampleStatusText();
        int statusW = font().width(statusText);
        int currentStatusColor = getSampleStatusColor(skinTextColor);
        context.text(font(), Component.literal(statusText), panelX + 18, statusY, currentStatusColor, false);
        ...
        if (statusHovered) {
            ...
            if (hintX + hintW <= hintMaxRight) {
                context.text(font(), Component.literal(cycleHint), hintX, statusY, 0x99E9C46A, false);
            }
        }
    ...
    private void drawSkinLayerPreview(GuiGraphicsExtractor context, int previewLeft, int previewTop, int previewWidth) {
        LayerPreviewLayout layer = getLayerPreviewLayout(previewLeft, previewTop, previewWidth);
    ...
    private void drawChevronIcon(GuiGraphicsExtractor context, int boxLeft, int boxTop, boolean openUp, int color) {
    ...
    private void drawLayerCheckbox(GuiGraphicsExtractor context, int x, int y, boolean enabled, int color) {
        int openWidth = font().width("[");
        int innerWidth = font().width(enabled ? "x" : " ");
        context.text(font(), Component.literal("["), x, y, color, false);
        if (enabled) {
            context.text(font(), Component.literal("x"), x + openWidth, y - 1, color, false);
        } else {
            context.text(font(), Component.literal(" "), x + openWidth, y, color, false);
        }
        context.text(font(), Component.literal("]"), x + openWidth + innerWidth, y, color, false);
    }
    ...
    private void renderActionButton(
        ...
        context.text(font(), Component.literal(label), textX, textY, foreground, false);
    }

    private void renderColorSwatch(GuiGraphicsExtractor context, int x, int y, int width, int height, int color) {
    ...
    private void ensureTextures() {
        ...
        horizontalPaletteTextureId = Identifier.fromNamespaceAndPath("chunkloader", "panel_color_editor_palette_h");
        ...
        verticalPaletteTextureId = Identifier.fromNamespaceAndPath("chunkloader", "panel_color_editor_palette_v");
        ...
        checkerboardTextureId = Identifier.fromNamespaceAndPath("chunkloader", "panel_color_editor_checker");
    ...
    private void ensureOpacityGradientTexture(int rgb, int width, int height, boolean vertical) {
        ...
        opacityGradientTextureId = Identifier.fromNamespaceAndPath("chunkloader", "panel_color_editor_opacity");
    ...
    private void blitCheckerboardTiled(GuiGraphicsExtractor context, int x, int y, int width, int height) {
    ...
    private static void fillArgb(GuiGraphicsExtractor context, int left, int top, int right, int bottom, int color) {
        context.fill(left, top, right, bottom, color);
    }

    private static void drawOutline(GuiGraphicsExtractor context, int left, int top, int right, int bottom, int color) {
