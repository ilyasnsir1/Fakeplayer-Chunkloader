...
import de.chunkloader.client.NativeImageColorUtil;
...
final class PanelColorEditorOverlay {
    private static final WidgetSprites VANILLA_BUTTON_TEXTURES = new WidgetSprites(
    ...
    private DynamicTexture horizontalPaletteTexture;
    private ResourceLocation horizontalPaletteTextureId;
    private DynamicTexture verticalPaletteTexture;
    private ResourceLocation verticalPaletteTextureId;
    private DynamicTexture checkerboardTexture;
    private ResourceLocation checkerboardTextureId;
    private DynamicTexture opacityGradientTexture;
    private ResourceLocation opacityGradientTextureId;
    ...
    private void initHexField() {
        ...
        hexField = owner.addOverlayWidget(new EditBox(font(), hexBounds.left(), hexBounds.top(), hexBounds.width(), hexBounds.height(), Component.literal("Hex")));
        hexField.setMaxLength(7);
        hexField.setBordered(true);
        hexField.setHint(Component.literal("#RRGGBB").withStyle(ChatFormatting.GRAY));
        updateHexFieldValue();
        hexField.setResponder(text -> {
            ...
            if (!s.startsWith("#")) {
                ...
                int cursorPos = hexField.getCursorPosition();
                hexField.setValue(fixed);
                hexField.setCursorPosition(Math.max(1, cursorPos));
    ...
    private void ensureHexPrefixOnFocus() {
        ...
        String value = hexField.getValue();
        if (value == null || value.isBlank()) {
            updatingHexFieldFromCode = true;
            hexField.setValue("#");
            hexField.setCursorPosition(1);
            updatingHexFieldFromCode = false;
        } else if (!value.startsWith("#")) {
            updatingHexFieldFromCode = true;
            hexField.setValue("#" + value.replaceAll("[^0-9a-fA-F]", ""));
            hexField.setCursorPosition(Math.min(hexField.getValue().length(), Math.max(1, hexField.getCursorPosition() + 1)));
            updatingHexFieldFromCode = false;
        }
    }
    ...
    private void clearIncompleteHexOnBlur() {
        ...
        String value = hexField.getValue() == null ? "" : hexField.getValue().trim();
    ...
    void render(GuiGraphics context, int mouseX, int mouseY) {
        ...
        Component targetLabel = currentTarget != null
            ? Component.literal("Color: " + currentTarget.getDisplayName() + " (A:" + alpha(currentColor) + ")").withStyle(ChatFormatting.WHITE)
            : Component.literal("Select a UI element").withStyle(ChatFormatting.GRAY);

        if (layout.vertical()) {
            if (currentTarget == null) {
                Bounds hexBounds = layout.hexField();
                int labelX = hexBounds.left() + (hexBounds.width() - font().width(targetLabel)) / 2;
                int labelY = hexBounds.top() - font().lineHeight - 3;
                context.drawString(font(), targetLabel, labelX, labelY, 0xFFAAAAAA, false);
            } else {
                context.drawString(font(), Component.literal("Color: (A:" + alpha(currentColor) + ")").withStyle(ChatFormatting.GRAY), layout.cardLeft(), layout.cardTop() + 2, 0xFFAAAAAA, false);
                String[] words = currentTarget.getDisplayName().split(" ");
                int curY = layout.cardTop() + 2 + font().lineHeight + 1;
                for (String word : words) {
                    context.drawString(font(), Component.literal(word).withStyle(ChatFormatting.WHITE), layout.cardLeft(), curY, 0xFFFFFFFF, false);
                    curY += font().lineHeight + 1;
                }
            }
        } else {
            int paletteWidth = layout.paletteGridCols() * layout.paletteCellWidth();
            int labelX = layout.paletteLeft() + (paletteWidth - font().width(targetLabel)) / 2;

            int labelY = Math.max(layout.cardTop(), (layout.paletteTop() - font().lineHeight) / 2);
            context.drawString(font(), targetLabel, labelX, labelY, 0xFFFFFFFF, false);
        }
        ...
        context.drawString(font(), Component.literal("<").withStyle(ChatFormatting.BOLD), leftArrowX + (arrowW - font().width("<")) / 2, arrowY + (arrowH - font().lineHeight) / 2, leftColor, false);

        int rightColor = rightHovered ? 0xFFFFDD55 : 0xBBFFFFFF;
        context.drawString(font(), Component.literal(">").withStyle(ChatFormatting.BOLD), rightArrowX + (arrowW - font().width(">")) / 2, arrowY + (arrowH - font().lineHeight) / 2, rightColor, false);
    ...
    boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (getHelpMarkBounds().contains(ix, iy)) {
            Minecraft.getInstance().setScreen(new PanelColorHelpScreen(owner));
            return true;
        }
        ...
        Integer paletteColor = getPaletteColorAt(ix, iy, layout);
        ...
        if (layout.opacitySlider().contains(ix, iy)) {
            if (currentTarget != null) {
                setOpacityFromSlider(ix, iy, layout);
                opacitySliderDragging = true;
            }
            return true;
        }

        if (layout.saveButton().contains(ix, iy)) {
        ...
        if (layout.resetButton().contains(ix, iy)) {
            if (!canReset()) {
                return true;
            }
            focusedControl = 2;
        ...
        if (layout.cancelButton().contains(ix, iy)) {
            focusedControl = 3;
            updateFocusState();
            owner.closePanelColorEditor();
            return true;
        ...
        if (currentPage == 2) {
            PanelColorTarget skinTarget = getSkinPreviewTargetAt(ix, iy);
            if (skinTarget != null) {
        ...
        ChunkMapScreen.PanelColorTargetHit targetHit = owner.getPanelColorTargetHit(ix, iy);
    ...
    boolean mouseDragged(double mouseX, double mouseY, int button) {
        if (opacitySliderDragging) {
            setOpacityFromSlider((int) mouseX, (int) mouseY, getLayout());
        }
        return true;
    }
    ...
    boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (hexField != null && hexField.isFocused()) {
            if (key == 259 && hexField.getCursorPosition() <= 1) {
                return true;
            }
            if (key == 257) {
                if (currentTarget == null) {
                ...
                int currentAlpha = alpha(getDraftColor(currentTarget));
                Integer color = parseHexColor(hexField.getValue(), currentAlpha);
                if (color != null) {
                    putDraftColor(currentTarget, color);
                }
    ...
    private void renderPalette(GuiGraphics context, int mouseX, int mouseY, EditorLayout layout) {
        ...
        ResourceLocation textureId = layout.vertical() ? verticalPaletteTextureId : horizontalPaletteTextureId;
    ...
    private void renderOpacitySlider(GuiGraphics context, EditorLayout layout, int color) {
    ...
    private void renderVanillaButton(
        ...
        context.blitSprite(VANILLA_BUTTON_TEXTURES.get(active, hovered), x1, y1, width, height);
        int foreground = active ? 0xFFFFFFFF : 0xFFA0A0A0;
        int textX = x1 + (width - font().width(text)) / 2;
        int textY = y1 + (height - font().lineHeight) / 2 + 1;
        context.drawString(font(), Component.literal(text), textX, textY, foreground, false);
    }
    ...
    private PanelColorTarget getSkinPreviewTargetAt(int mouseX, int mouseY) {
        ...
        Component titleComp = Component.literal("Select Skin").withStyle(ChatFormatting.BOLD);
        int titleW = font().width(titleComp);
        int titleX = panelX + (panelWidth - titleW) / 2 + 1;
        int titleY = panelY + 12;
        if (mouseX >= titleX - 4 && mouseX <= titleX + titleW + 4 && mouseY >= titleY - 2 && mouseY < titleY + font().lineHeight + 2) {
        ...
        int playerW = font().width("Player: Fakeplayer4");
        ...
        if (mouseX >= rowX && mouseX < rowX + fieldWidth && mouseY >= fileFieldY && mouseY < fileFieldY + 20) {
            ...
            int pathLabelW = font().width("Path to a skin PNG");
            int textW = font().width("/custom.png");
        ...
        int hintW = font().width("No skin PNG loaded yet");
        ...
        int statusY = panelY + panelHeight - 58;
        int statusW = font().width(getSampleStatusText());
    ...
    private void drawSkinScreenPreview(GuiGraphics context, int mouseX, int mouseY) {
        int panelWidth = getPreviewPanelWidth();
        int panelHeight = Math.min(370, Math.max(1, owner.height - 24));
        int panelX = getPreviewPanelX(panelWidth);
        int panelY = (owner.height - panelHeight) / 2;
        ...
        int titleX = panelX + (panelWidth - font().width(title)) / 2 + 1;
        int titleY = panelY + 12;
        context.drawString(font(), title, titleX, titleY, skinTitleColor, false);

        context.drawString(font(), Component.literal("Player: Fakeplayer4"), panelX + 18, panelY + 35, skinPlayerNameColor, false);
        context.drawString(font(), Component.literal("Skin PNG"), panelX + 18, panelY + 52, skinTextColor, false);
        ...
        context.drawString(font(), Component.literal("Path to a skin PNG"), rowX + 4, fileFieldY + 7, skinPathPlaceholderColor, false);
        int pathLabelW = font().width("Path to a skin PNG");
        context.drawString(font(), Component.literal("/custom.png"), rowX + 6 + pathLabelW, fileFieldY + 7, skinPathTextColor, false);
        ...
        context.drawString(font(), Component.literal("3D Preview"), previewLeft, previewTop - 13, skinTextColor, false);
        ...
        context.drawString(font(), Component.literal(statusText), panelX + 18, statusY, currentStatusColor, false);

        boolean statusHovered = mouseX >= panelX + 14 && mouseX <= panelX + 18 + statusW + 4
            && mouseY >= statusY - 4 && mouseY <= statusY + 12;
        if (statusHovered) {
            String cycleHint = "Double-click to cycle";
            int hintW = font().width(cycleHint);
            int hintX = panelX + 18 + statusW + 10;
            int hintMaxRight = panelX + panelWidth - 18;
            if (hintX + hintW <= hintMaxRight) {
                context.drawString(font(), Component.literal(cycleHint), hintX, statusY, 0x99E9C46A, false);
            }
        }
        ...
        if (hoveredSkinTarget != null) {
            switch (hoveredSkinTarget) {
                case SKIN_TITLE -> {
                    Component tComp = Component.literal("Select Skin").withStyle(ChatFormatting.BOLD);
                    int tw = font().width(tComp);
                    int tx = panelX + (panelWidth - tw) / 2 + 1;
                    int ty = panelY + 12;
                    drawOutline(context, tx - 4, ty - 2, tx + tw + 4, ty + font().lineHeight + 2, 0xFFE9C46A);
                }
                case SKIN_DIVIDER -> {
                ...
                case SKIN_PLAYER_NAME -> drawOutline(context, panelX + 15, panelY + 32, panelX + 18 + font().width("Player: Fakeplayer4") + 4, panelY + 46, 0xFFE9C46A);
    ...
    private void drawSkinLayerPreview(GuiGraphics context, int previewLeft, int previewTop, int previewWidth) {
        LayerPreviewLayout layer = getLayerPreviewLayout(previewLeft, previewTop, previewWidth);
        ...
        int checkboxWidth = font().width("[x]");
        ...
        for (int i = 0; i < SkinLayerMask.EDITABLE_PARTS.length; i++) {
            ...
            context.drawString(
                font(),
                Component.literal(SkinLayerMask.label(SkinLayerMask.EDITABLE_PARTS[i])),
    ...
    private void drawLayerCheckbox(GuiGraphics context, int x, int y, boolean enabled, int color) {
        int openWidth = font().width("[");
        int innerWidth = font().width(enabled ? "x" : " ");
        context.drawString(font(), Component.literal("["), x, y, color, false);
        if (enabled) {
            context.drawString(font(), Component.literal("x"), x + openWidth, y - 1, color, false);
        } else {
            context.drawString(font(), Component.literal(" "), x + openWidth, y, color, false);
        }
        context.drawString(font(), Component.literal("]"), x + openWidth + innerWidth, y, color, false);
    }
    ...
    private void renderActionButton(
        ...
        int textY = bounds.top() + (bounds.height() - font().lineHeight) / 2 + 1;
        context.drawString(font(), Component.literal(label), textX, textY, foreground, false);
    }

    private void renderColorSwatch(GuiGraphics context, int x, int y, int width, int height, int color) {
    ...
    private void ensureTextures() {
        ...
        for (int row = 0; row < PALETTE_ROWS; row++) {
            for (int col = 0; col < PALETTE_COLUMNS; col++) {
                horizontal.setPixelRGBA(col, row, NativeImageColorUtil.argbToNative(0xFF000000 | getPaletteRgb(col, row)));
            }
        }
        horizontalPaletteTexture = new DynamicTexture(horizontal);
        horizontalPaletteTextureId = ResourceLocation.fromNamespaceAndPath("chunkloader", "panel_color_editor_palette_h");
        client.getTextureManager().register(horizontalPaletteTextureId, horizontalPaletteTexture);
        ...
        for (int row = 0; row < PALETTE_COLUMNS; row++) {
            for (int col = 0; col < PALETTE_ROWS; col++) {
                vertical.setPixelRGBA(col, row, NativeImageColorUtil.argbToNative(0xFF000000 | getPaletteRgb(row, col)));
            }
        }
        verticalPaletteTexture = new DynamicTexture(vertical);
        verticalPaletteTextureId = ResourceLocation.fromNamespaceAndPath("chunkloader", "panel_color_editor_palette_v");
        client.getTextureManager().register(verticalPaletteTextureId, verticalPaletteTexture);
        ...
        for (int y = 0; y < 12; y++) {
            for (int x = 0; x < 12; x++) {
                ...
                checker.setPixelRGBA(x, y, NativeImageColorUtil.argbToNative(light ? 0xFFFFFFFF : 0xFFB0B0B0));
            }
        }
        checkerboardTexture = new DynamicTexture(checker);
        checkerboardTextureId = ResourceLocation.fromNamespaceAndPath("chunkloader", "panel_color_editor_checker");
        client.getTextureManager().register(checkerboardTextureId, checkerboardTexture);
    ...
    private void ensureOpacityGradientTexture(int rgb, int width, int height, boolean vertical) {
        ...
        if (
            ...
            NativeImage image = opacityGradientTexture.getPixels();
            if (image != null) {
        ...
        opacityGradientTexture = new DynamicTexture(image);
        opacityGradientTextureId = ResourceLocation.fromNamespaceAndPath("chunkloader", "panel_color_editor_opacity");
        client.getTextureManager().register(opacityGradientTextureId, opacityGradientTexture);
        cachedOpacityRgb = rgb;
        cachedOpacityWidth = width;
        cachedOpacityHeight = height;
        cachedOpacityVertical = vertical;
    ...
    private static void fillOpacityGradient(NativeImage image, int rgb, int width, int height, boolean vertical) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                image.setPixelRGBA(x, y, NativeImageColorUtil.argbToNative((alphaValue << 24) | (rgb & 0x00FFFFFF)));
            }
        }
    }

    private void releaseOpacityGradientTexture(Minecraft client) {
        if (opacityGradientTextureId != null && client != null && client.getTextureManager() != null) {
            client.getTextureManager().release(opacityGradientTextureId);
            opacityGradientTextureId = null;
        }
    ...
    private void cleanupTextures() {
        ...
        if (client != null && client.getTextureManager() != null) {
            if (horizontalPaletteTextureId != null) {
                client.getTextureManager().release(horizontalPaletteTextureId);
                horizontalPaletteTextureId = null;
            }
            if (verticalPaletteTextureId != null) {
                client.getTextureManager().release(verticalPaletteTextureId);
    ...
    private void blitCheckerboardTiled(GuiGraphics context, int x, int y, int width, int height) {
    ...
    private EditorLayout getHorizontalLayout() {
        ...
        int paletteTop = cardTop + font().lineHeight + 4;
    ...
    private EditorLayout getVerticalLayout() {
        ...
        int titleHeight = totalTitleLines * (font().lineHeight + 1) + 2;
    ...
    private static void fillArgb(GuiGraphics context, int left, int top, int right, int bottom, int color) {
        context.fill(left, top, right, bottom, color);
    }

    private static void drawOutline(GuiGraphics context, int left, int top, int right, int bottom, int color) {
    ...
    private Bounds getHelpMarkBounds() {
        int markX = 8;
        int markY = 8;
        int markW = Math.max(10, font().width("?"));
