...
public final class CustomFakePlayerSkinCache {
    ...
    private static Path syncedSkinsDirectory() {
        ...
        return client.runDirectory.toPath().resolve("chunkloader").resolve("synced_skins");
    }
    ...
    private static void applyLegacyTransparencyFix(NativeImage skinImage, int x0, int y0, int x1, int y1) {
        ...
        for (int x = x0; x < x1; x++) {
            for (int y = y0; y < y1; y++) {
                int color = skinImage.getColor(x, y);
                skinImage.setColor(x, y, color & 0x00FFFFFF);
            }
        }
    }
    ...
    private static void setNoAlpha(NativeImage skinImage, int x0, int y0, int x1, int y1) {
        for (int x = x0; x < x1; x++) {
            for (int y = y0; y < y1; y++) {
                int color = skinImage.getColor(x, y);
                skinImage.setColor(x, y, 0xFF000000 | (color & 0x00FFFFFF));
            }
        }
    }
