...
public final class CustomFakePlayerSkinCache {
    ...
    public static void refreshBindings() {
        ...
        if (client.level != null) {
            for (var player : client.level.players()) {
                ...
                if (profile != null) {
                    ...
                    bindPlayer(profile.getId(), plainName != null ? plainName : profile.getName());
                }
            }
        }
        ...
        for (var playerInfo : connection.getOnlinePlayers()) {
            ...
            if (profile != null) {
                bindPlayer(profile.getId(), profile.getName());
            }
        }
    }
    ...
    private static CustomSkin loadSkin(String normalizedPath) throws IOException {
        ...
        DynamicTexture texture = new DynamicTexture(skinImage);
        try {
    ...
    private static int chunkloader$alpha(int color) {
        return (color >>> 24) & 0xFF;
    }

    private static int chunkloader$opaque(int color) {
    ...
    private static SkinModelType detectModel(NativeImage skinImage) {
        ...
        return chunkloader$alpha(skinImage.getPixelRGBA(54, 20)) == 0 ? SkinModelType.SLIM : SkinModelType.WIDE;
    }
    ...
    private static void applyLegacyTransparencyFix(NativeImage skinImage, int x0, int y0, int x1, int y1) {
        for (int x = x0; x < x1; x++) {
            for (int y = y0; y < y1; y++) {
                if (chunkloader$alpha(skinImage.getPixelRGBA(x, y)) < 128) {
                    return;
                }
            }
        }
        ...
        for (int x = x0; x < x1; x++) {
            for (int y = y0; y < y1; y++) {
                skinImage.setPixelRGBA(x, y, skinImage.getPixelRGBA(x, y) & 0x00FFFFFF);
            }
        }
    }
    ...
    private static void setNoAlpha(NativeImage skinImage, int x0, int y0, int x1, int y1) {
        for (int x = x0; x < x1; x++) {
            for (int y = y0; y < y1; y++) {
                skinImage.setPixelRGBA(x, y, chunkloader$opaque(skinImage.getPixelRGBA(x, y)));
            }
        }
    }
