...
public final class CustomFakePlayerSkinCache {
    ...
    public record CustomSkin(Identifier assetId, Identifier textureId, PlayerModelType model) {
    }
    ...
    private static void bindPlayer(UUID playerUuid, String playerName) {
        ...
        try {
            ...
            if (failedPaths.add(path)) {
                ChunkloaderForgeMod.LOGGER.warn("Unable to load custom skin '{}': {}", path, e.getMessage());
            }
        }
    }
    ...
    private static CustomSkin loadSkin(String normalizedPath) throws IOException {
        ...
        PlayerModelType model = detectModel(skinImage);
        skinImage = normalizeSkinImage(skinImage);
        Identifier assetId = Identifier.fromNamespaceAndPath(
            ChunkloaderForgeMod.MOD_ID,
    ...
    private static PlayerModelType detectModel(NativeImage skinImage) {
        if (skinImage.getHeight() == 32) {
            return PlayerModelType.WIDE;
        }
        return ARGB.alpha(skinImage.getPixel(54, 20)) == 0 ? PlayerModelType.SLIM : PlayerModelType.WIDE;
    }
